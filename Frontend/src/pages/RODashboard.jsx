...
const RODashboard = () => {
  // 1. ADDED TRIGGER STATE FOR LIVE UPDATES
  ...
  // 2. FETCH DATA WHENEVER TRIGGER CHANGES
  useEffect(() => {
    fetchAlerts();
  }, [refreshTrigger]);
  ...
  useEffect(() => {
    // Uses Render URL if deployed, otherwise falls back to local!
    const wsUrl = import.meta.env.VITE_WS_URL || 'ws://127.0.0.1:8000/api/ws/alerts';
    const ws = new WebSocket(wsUrl);

    ws.onopen = () => console.log("🟢 Mobile RO: Connected to Live Alerts WebSocket");

    ws.onmessage = (event) => {
      try {
        const data = JSON.parse(event.data);
        if (data.event === 'REFRESH_ALERTS') {
          console.log("⚡ Live Update Received! Refreshing Mobile RO...");
          setRefreshTrigger(prev => prev + 1); // Trigger the re-fetch!
        }
      } catch (err) {
  ...
  const fetchAlerts = async () => {
    try {
      ...
      // Filter for unresolved/pending alerts if needed, or just set all
  ...
  const handleResolve = (alertId) => {
    // Note: In a real flow, you'd also want to make an API call to resolve this in the DB here!
        ...
        <div className="p-6 space-y-6 max-w-3xl mx-auto">
          ...
          <div className="space-y-4">
            {alerts.map((alert) => (
              <div
                key={alert.id}
                className="bg-[#151c21]/85 backdrop-blur-md rounded-xl p-5 border border-[#232f36] transition-all hover:border-emerald-500/30"
              >
                <div className="flex justify-between mb-2">
                  <span className={`text-xs font-bold ${alert.status === 'ACTION_TAKEN' ? 'text-yellow-500' : 'text-red-400'}`}>
                    {alert.status || alert.severity || 'HIGH'}
                  </span>
                  <span className="text-gray-500 text-xs">{alert.id || alert.log_id?.slice(-6)}</span>
                </div>
                <h3 className="text-white font-bold">{alert.industry_id}</h3>
                <p className="text-gray-400 text-xs flex items-center gap-1 mt-1">
                  <MapIcon className="w-3 h-3" />
                  {alert.location || alert.location_id}
                </p>
                <div className="mt-3 text-red-400 font-semibold">
                  {alert.parameter} : {alert.exceeded_value} <span className="text-gray-500 text-xs font-normal">(Limit: {alert.allowed_value})</span>
                </div>
