...
const Login = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans relative overflow-hidden"
         style={{ backgroundColor: '#0b1114', color: '#94a3b8' }}>
      ...
      <nav className="absolute top-0 left-0 w-full h-[72px] backdrop-blur-md flex justify-between items-center px-8 z-10"
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-emerald-500 font-bold">Authentication Portal</p>
          </div>
        </div>
        <div className="hidden sm:flex items-center gap-6 text-sm font-medium">
          <a href="#" className="text-slate-400 hover:text-emerald-500 transition-colors">Help</a>
          <a href="#" className="text-slate-400 hover:text-emerald-500 transition-colors">English</a>
        </div>
      </nav>
        ...
        <div className="w-full lg:w-[45%] mb-12 lg:mb-0">
          <div className="backdrop-blur-xl border shadow-xl p-10 rounded-2xl"
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="flex-1 border rounded-xl p-5 hover:border-emerald-500/50 transition-all"
                   style={{ backgroundColor: '#0b1114', borderColor: 'rgba(16, 185, 129, 0.2)' }}>
                <Shield className="w-6 h-6 text-emerald-400 mb-3" />
                <h3 className="font-black mb-1 text-white">Secure Access</h3>
                <p className="text-xs font-bold text-slate-400">256-bit SSL Encrypted</p>
              </div>
              <div className="flex-1 border rounded-xl p-5 hover:border-emerald-500/50 transition-all"
                   style={{ backgroundColor: '#0b1114', borderColor: 'rgba(16, 185, 129, 0.2)' }}>
                <BarChart3 className="w-6 h-6 text-emerald-400 mb-3" />
                <h3 className="font-black mb-1 text-white">Real-time Data</h3>
                <p className="text-xs font-bold text-slate-400">Live Industry Feeds</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="backdrop-blur-xl border rounded-2xl p-8 shadow-2xl"
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              <div className="space-y-3">
                <label className="text-sm font-bold text-slate-400">Select Your Role</label>
                <div className="grid grid-cols-2 gap-3">
                  {roles.map((role) => {
                    const Icon = role.icon;
                    return (
                      <button key={role.id} type="button" onClick={() => setSelectedRole(role.id)}
                        className={`flex flex-col items-center justify-center p-3 rounded-xl border text-[10px] font-black tracking-tighter transition-all ${
                          selectedRole === role.id
                            ? 'border-emerald-500 bg-emerald-500/10 text-emerald-400'
                            : 'border-[rgba(16,185,129,0.2)] bg-[#0b1114] text-slate-400 hover:bg-emerald-500/5 hover:border-emerald-500/40'
                        }`}>
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-bold text-slate-400">Username or Email</label>
                <div className="relative">
                  ...
                  <input type="text" required value={username} onChange={(e) => setUsername(e.target.value)}
                    ...
                    className="theme-input block w-full pl-11 pr-4 py-3 rounded-xl outline-none transition-all" />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center">
                  ...
                  <a href="#" className="text-xs text-emerald-500 font-bold hover:underline">Forgot Password?</a>
                </div>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-500">
              ...
              <div className="border-t border-[rgba(16,185,129,0.2)]"></div>

              <button type="button" onClick={() => navigate('/public-portal')}
                className="w-full flex items-center justify-center gap-2 border font-bold py-3 px-4 rounded-xl transition-all hover:bg-emerald-500/10 text-white"
                style={{ backgroundColor: '#0b1114', borderColor: 'rgba(16, 185, 129, 0.2)' }}>
